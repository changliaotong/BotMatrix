...
### 3.4 MCP 标准化 (MCP Standardization)
- **MCP Host**: BotNexus 支持连接外部 MCP Server，实现能力的无限扩展。
- **MCP Server**: 现有的 Skill Center 能力通过 MCP 协议对外暴露。

## 4. 阶段性规划

### 第一阶段：基础设施增强 (已完成)
- [x] **完善 Skill 定义**：支持 JSON Schema 参数描述。
- [x] **实现 PII 隐私脱敏**：保护用户敏感数据。
- [x] **MCP 基础层实现**：初步建立 MCP 管理器与协议定义。

### 第二阶段：AI 深度集成 (进行中)
- [ ] **MCP Server 落地**: 将现有 Skill 导出为 MCP Tools。
- [ ] **对话补全**：实现缺失参数的 AI 自动追问机制。
- [ ] **GraphRAG 优化**: 增强跨文档关系推理能力。

### 第三阶段：生态与闭环 (规划中)
- [ ] **Vision 3.0 愿景落地**: 实现多智能体 Swarm 调度与长上下文协同。
