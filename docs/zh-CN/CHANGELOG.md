...
## v1.9.0 (2026-01-02)
*   **🛡️ 插件安全与权限架构 (Plugin Security & Permissions)**:
    - **精细化权限校验**: 实现了基于 `plugin.json` 的动作白名单校验机制，支持 `*` 通配符权限，确保插件只能执行其声明范围内的动作。
    - **安全审计日志**: 增加了插件违规操作的实时拦截与详细日志记录，提升了系统对恶意/异常插件的防御能力。
*   **🌐 跨插件混合路由 (Inter-Plugin Hybrid Routing)**:
    - **全链路技能调用**: 重构了 `Skill` 调用机制，支持外部进程插件与内部 Go 插件之间的双向透明调用。
    - **自动能力报备**: 完善了插件能力的动态发现与报备逻辑，所有内部与外部插件的技能现在均能自动同步至 BotNexus 中心。
    - **异步转同步响应**: 建立了完整的跨插件响应闭环，支持通过 `correlation_id` 实现复杂的异步协作流程。
*   **🛠️ 开发者工具链升级 (Developer Tooling)**:
    - **bm-cli 初始化增强**: 升级了插件初始化模版，自动生成包含标准权限声明、意图定义及示例代码的开发环境。
    - **规范化 Manifest**: 统一了插件 `plugin.json` 与核心系统配置结构，新增了 `Permissions`、`RunOn`、`Intents` 等关键生产力字段。
