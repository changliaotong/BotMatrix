...
func (pb *PluginBridge) Reload() error {
	...
	// 加载 TitlePlugin
	titlePlugin := plugins.NewTitlePlugin()
	if err := pb.pluginManager.LoadPluginModule(titlePlugin, pb.server); err != nil {
		log.Errorf("加载 TitlePlugin 失败: %v", err)
	}

	// 加载 WelcomePlugin
	if plugins.GlobalStore != nil {
		welcomePlugin := plugins.NewWelcomePlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(welcomePlugin, pb.server); err != nil {
			log.Errorf("加载 WelcomePlugin 失败: %v", err)
		}
	} else {
		log.Warnf("Skipping WelcomePlugin loading because GlobalStore is nil")
	}

	// 加载 GuessGamePlugin
	if plugins.GlobalStore != nil {
		guessPlugin := plugins.NewGuessGamePlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(guessPlugin, pb.server); err != nil {
			log.Errorf("加载 GuessGamePlugin 失败: %v", err)
		}

		// 加载 JielongPlugin
		jielongPlugin := plugins.NewJielongPlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(jielongPlugin, pb.server); err != nil {
			log.Errorf("加载 JielongPlugin 失败: %v", err)
		}
	} else {
		log.Warnf("Skipping GuessGamePlugin/JielongPlugin loading because GlobalStore is nil")
	}

	// 加载 AdminPlugin
	if plugins.GlobalGroupService != nil {
		adminPlugin := plugins.NewAdminPlugin(plugins.GlobalGroupService, plugins.GlobalUserService, plugins.GlobalEconomyService)
		if err := pb.pluginManager.LoadPluginModule(adminPlugin, pb.server); err != nil {
			log.Errorf("加载 AdminPlugin 失败: %v", err)
		}
	} else {
		log.Warnf("Skipping AdminPlugin loading because GlobalGroupService is nil")
	}

	// 加载 RedBluePlugin
	if plugins.GlobalStore != nil && plugins.GlobalGORMDB != nil {
		blockchainService := services.NewBlockchainService(plugins.GlobalGORMDB)
		redBluePlugin := plugins.NewRedBluePlugin(plugins.GlobalStore, blockchainService)
		if err := pb.pluginManager.LoadPluginModule(redBluePlugin, pb.server); err != nil {
			log.Errorf("加载 RedBluePlugin 失败: %v", err)
		}
	} else {
		log.Warnf("Skipping RedBluePlugin loading because GlobalStore or GlobalGORMDB is nil")
	}

	// 加载 TeachPlugin (Q&A)
	if plugins.GlobalStore != nil {
		teachPlugin := plugins.NewTeachPlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(teachPlugin, pb.server); err != nil {
			log.Errorf("加载 TeachPlugin 失败: %v", err)
		}

		// 加载 RedBluePlugin
		redBluePlugin := plugins.NewRedBluePlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(redBluePlugin, pb.server); err != nil {
			log.Errorf("加载 RedBluePlugin 失败: %v", err)
		}
		...
		log.Warnf("Skipping TeachPlugin loading because GlobalStore is nil")
	}
...
func (pb *PluginBridge) LoadInternalPlugins() error {
	...
	// 加载 EntertainmentPlugin (Non-core features migration)
	entPlugin := plugins.NewEntertainmentPlugin(plugins.GlobalStore)
	if err := pb.pluginManager.LoadPluginModule(entPlugin, pb.server); err != nil {
		log.Errorf("加载 EntertainmentPlugin 失败: %v", err)
	}

	// 加载 ToolsPlugin
	toolsPlugin := plugins.NewToolsPlugin()
	if err := pb.pluginManager.LoadPluginModule(toolsPlugin, pb.server); err != nil {
		log.Errorf("加载 ToolsPlugin 失败: %v", err)
	}

	// 加载 EconomyPlugin (New)
	economyPlugin := &plugins.EconomyPlugin{}
	if err := pb.pluginManager.LoadPluginModule(economyPlugin, pb.server); err != nil {
		log.Errorf("加载 EconomyPlugin 失败: %v", err)
	}

	// 加载 WeatherPlugin
	weatherPlugin := plugins.NewWeatherPlugin()
	if err := pb.pluginManager.LoadPluginModule(weatherPlugin, pb.server); err != nil {
		log.Errorf("加载 WeatherPlugin 失败: %v", err)
	}

	// 加载 FortunePlugin
	fortunePlugin := plugins.NewFortunePlugin()
	if err := pb.pluginManager.LoadPluginModule(fortunePlugin, pb.server); err != nil {
		log.Errorf("加载 FortunePlugin 失败: %v", err)
	}

	// 加载 TranslatePlugin
	translatePlugin := plugins.NewTranslatePlugin(pb.server.GetConfig().Translate)
	if err := pb.pluginManager.LoadPluginModule(translatePlugin, pb.server); err != nil {
		log.Errorf("加载 TranslatePlugin 失败: %v", err)
	}

	// 加载 MusicPlugin
	musicPlugin := plugins.NewMusicPlugin()
	if err := pb.pluginManager.LoadPluginModule(musicPlugin, pb.server); err != nil {
		log.Errorf("加载 MusicPlugin 失败: %v", err)
	}

	// 加载 HelpPlugin
	helpPlugin := plugins.NewHelpPlugin()
	if err := pb.pluginManager.LoadPluginModule(helpPlugin, pb.server); err != nil {
		log.Errorf("加载 HelpPlugin 失败: %v", err)
	}

	// 加载 GreetingPlugin
	if plugins.GlobalStore != nil {
		greetingPlugin := plugins.NewGreetingPlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(greetingPlugin, pb.server); err != nil {
			log.Errorf("加载 GreetingPlugin 失败: %v", err)
		}

		// 加载 GuessGamePlugin (Migrated)
		guessPlugin := plugins.NewGuessGamePlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(guessPlugin, pb.server); err != nil {
			log.Errorf("加载 GuessGamePlugin 失败: %v", err)
		}

		// 加载 RedBluePlugin (Migrated)
		redBluePlugin := plugins.NewRedBluePlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(redBluePlugin, pb.server); err != nil {
			log.Errorf("加载 RedBluePlugin 失败: %v", err)
		}

		// 加载 JielongPlugin (Migrated)
		jielongPlugin := plugins.NewJielongPlugin(plugins.GlobalStore)
		if err := pb.pluginManager.LoadPluginModule(jielongPlugin, pb.server); err != nil {
			log.Errorf("加载 JielongPlugin 失败: %v", err)
		}
	} else {
		log.Warnf("Skipping GreetingPlugin and Games loading because GlobalStore is nil")
	}

	// 加载 AdminPlugin
	// Ensure GlobalStore is initialized before this
	if plugins.GlobalStore != nil {
		groupService := services.NewGroupService(plugins.GlobalStore)
		userService := services.NewUserService(plugins.GlobalStore)
		economyService := services.NewEconomyService(plugins.GlobalStore)
		adminPlugin := plugins.NewAdminPlugin(groupService, userService, economyService)
		if err := pb.pluginManager.LoadPluginModule(adminPlugin, pb.server); err != nil {
			log.Errorf("加载 AdminPlugin 失败: %v", err)
		}
	} else {
		log.Warnf("Skipping AdminPlugin loading because GlobalStore is nil")
	}
	...
	pb.server.SetActionRouter(func(pluginID string, action string, payload map[string]any) (any, error) {
		...
		if pluginID != "" {
			...
			if _, ok := internalPlugins[pluginID]; ok {
				...
				var platform string
				if v, ok := payload["bot_id"]; ok {
					botUin = utils.ToInt64(v)
				}
				if v, ok := payload["group_id"]; ok {
					groupID = utils.ToInt64(v)
				}
				if v, ok := payload["user_id"]; ok {
					userID = utils.ToInt64(v)
				}
				if v, ok := payload["platform"]; ok {
					platform = fmt.Sprint(v)
				}
				ctx := server.NewWorkerContext(botUin, groupID, userID, platform)
	...
	pb.pluginManager.RegisterActionHandler(func(p *core.Plugin, a *core.Action) {
		...
		if a.Type == "call_skill" {
			go func() {
				...
				var botUin, groupID, userID int64
				var platform string
				if v, ok := a.Payload["bot_id"]; ok {
					botUin = utils.ToInt64(v)
				}
				if v, ok := a.Payload["group_id"]; ok {
					groupID = utils.ToInt64(v)
				}
				if v, ok := a.Payload["user_id"]; ok {
					userID = utils.ToInt64(v)
				}
				if v, ok := a.Payload["platform"]; ok {
					platform = fmt.Sprint(v)
				}
				ctx := server.NewWorkerContext(botUin, groupID, userID, platform)
...
func (w *ExternalPluginWrapper) Init(robot core.Robot) {
	...
	for _, cap := range w.plugin.Config.Capabilities {
		capName := cap.Name
		...
		if capability.Description == "" {
			capability.Description = fmt.Sprintf("Capability: %s", capName)
		}
		if capability.Usage == "" {
			capability.Usage = fmt.Sprintf("Directly call capability %s", capName)
		}
		w.registerSkill(robot, capability)
	}
}
...
func (w *ExternalPluginWrapper) GetSkills() []core.SkillCapability {
	...
	for _, cap := range w.plugin.Config.Capabilities {
		...
		skills = append(skills, capability)
	}
	return skills
}
