    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16 items-center">
        <router-link to="/" class="flex items-center gap-2 group cursor-pointer">
          <div class="w-8 h-8 bg-cyan-500 rounded-lg flex items-center justify-center font-bold text-slate-900 group-hover:rotate-12 transition-transform">M</div>
          <span class="text-xl font-bold tracking-tight text-white">BotMatrix</span>
        </router-link>
        
        <!-- Desktop Nav -->
        <div class="hidden md:flex items-center gap-8 text-sm font-medium text-slate-400">
          <router-link to="/" class="hover:text-cyan-400 transition-colors" :class="{ 'text-cyan-400': route.path === '/' }">首页</router-link>
          
          <!-- Bots Dropdown -->
          <div class="relative group">
            <button class="flex items-center gap-1 hover:text-cyan-400 transition-colors py-4" :class="{ 'text-cyan-400': route.path.startsWith('/bots') }">
              官方机器人
            ...
            <div class="absolute top-full left-0 w-48 py-2 bg-slate-800 border border-slate-700 rounded-xl shadow-xl opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all">
              <router-link to="/bots/early-meow" class="block px-4 py-2 hover:bg-slate-700 hover:text-cyan-400">早喵机器人</router-link>
          ...
          <router-link to="/docs" class="hover:text-cyan-400 transition-colors" :class="{ 'text-cyan-400': route.path === '/docs' }">文档中心</router-link>
          <router-link to="/news" class="hover:text-cyan-400 transition-colors" :class="{ 'text-cyan-400': route.path === '/news' }">动态更新</router-link>
          <router-link to="/pricing" class="hover:text-cyan-400 transition-colors" :class="{ 'text-cyan-400': route.path === '/pricing' }">版本计划</router-link>
          <router-link to="/about" class="hover:text-cyan-400 transition-colors" :class="{ 'text-cyan-400': route.path === '/about' }">关于我们</router-link>
          
          <router-link :to="authStore.isAuthenticated ? '/console' : '/login'" class="px-5 py-2 bg-cyan-500 hover:bg-cyan-400 text-slate-900 rounded-full transition-all font-bold">
