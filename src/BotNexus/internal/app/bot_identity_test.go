...
func (m *MockKnowledgeBase) Search(ctx context.Context, query string, limit int, filter *tasks.SearchFilter) ([]tasks.DocChunk, error) {
	if strings.Contains(query, "架构") {
		return []tasks.DocChunk{
			{
				ID:      "chunk_1",
				Source:  "DOCS.md",
...
func (m *MockAIServiceForIdentity) ChatStream(ctx context.Context, modelID uint, messages []ai.Message, tools []ai.Tool) (<-chan ai.ChatStreamResponse, error) {
	return nil, nil
}

func (m *MockAIServiceForIdentity) Chat(ctx context.Context, modelID uint, messages []ai.Message, tools []ai.Tool) (*ai.ChatResponse, error) {
...
func (m *MockAIServiceForIdentity) CreateEmbedding(ctx context.Context, modelID uint, input any) (*ai.EmbeddingResponse, error) {
	return nil, nil
}
