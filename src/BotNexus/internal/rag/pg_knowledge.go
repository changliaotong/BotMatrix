...
type PostgresKnowledgeBase struct {
	...
	aiSvc            tasks.AIService // 用于 Query Refinement 等高级功能
	aiModelID        uint            // 用于 AI 服务的模型 ID
}

func NewPostgresKnowledgeBase(db *gorm.DB, es EmbeddingService, aiSvc tasks.AIService, aiModelID uint) *PostgresKnowledgeBase {
...
// Search 执行语义搜索
func (p *PostgresKnowledgeBase) Search(ctx context.Context, query string, limit int, filter *tasks.SearchFilter) ([]tasks.DocChunk, error) {
	return p.SearchHybrid(ctx, query, limit, filter)
}

// SearchHybrid 执行混合搜索 (向量 + 关键词)
func (p *PostgresKnowledgeBase) SearchHybrid(ctx context.Context, query string, limit int, filter *tasks.SearchFilter) ([]tasks.DocChunk, error) {
	...
	if p.aiSvc != nil {
		...
		resp, err := p.aiSvc.Chat(ctx, p.aiModelID, []ai.Message{
	...
	var finalChunks []KnowledgeChunk
	...
	if len(finalChunks) > limit {
		finalChunks = finalChunks[:limit]
	}

	// 3. 转换为任务系统通用的 DocChunk 格式
	...
	// 这是一个简单的自省机制，剔除 AI 认为不相关的片段，提高最终回答的质量
	if p.aiSvc != nil && len(results) > 0 {
		fmt.Printf("[RAG 2.0] Starting Self-Reflection for %d chunks...\n", len(results))
		var filteredResults []tasks.DocChunk
		for _, res := range results {
			reflectionPrompt := fmt.Sprintf("你是一个质量控制专家。请判断以下检索到的内容是否能够回答用户的提问，或者与提问高度相关。\n用户提问：%s\n检索内容：%s\n如果相关，请只输出 YES，如果不相关，请只输出 NO。不要输出任何其他文字。", query, res.Content)

			resp, err := p.aiSvc.Chat(ctx, p.aiModelID, []ai.Message{
				{Role: ai.RoleUser, Content: reflectionPrompt},
			}, nil)
