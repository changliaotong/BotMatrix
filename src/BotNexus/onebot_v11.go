...
func parseV11Message(raw string) []common.MessageSegment {
	...
	for _, match := range matches {
		...
		if match[0] > lastIndex {
			...
			segments = append(segments, common.MessageSegment{
				Type: "text",
				Data: common.TextSegmentData{Text: unescapeV11(text)},
			})
		}
	...
	if lastIndex < len(raw) {
		segments = append(segments, common.MessageSegment{
			Type: "text",
			Data: common.TextSegmentData{Text: unescapeV11(raw[lastIndex:])},
		})
	}
