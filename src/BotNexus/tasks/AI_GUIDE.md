...
### 1.2 任务管理与取消 (Task Management)
您可以随时取消已创建的自动化任务，AI 支持模糊指代。
- **示例输入**: "取消刚才的任务" 或 "帮我把 ID 为 123 的任务停了"
- **AI 动作**:
    - 识别意图为 `cancel_task`
    - 自动关联最近创建的任务 ID 或解析指定 ID
    - 停止该任务的所有后续执行计划。

### 1.3 自然语言策略调整 (AI Policy)
通过对话管理整个机器人矩阵的状态。
...
除了自然语言对话，您也可以直接使用以下指令：

| 指令 | 说明 | 示例 |
| --- | --- | --- |
| `#确认 [DraftID]` | 确认执行 AI 生成的任务草稿 | `#确认 A1B2C3D4` |
| `#取消 [TaskID]` | 停止并禁用指定的自动化任务 | `#取消 123` |
| `取消 [TaskID]` | 同上（不带井号） | `取消 123` |
