...
func (tm *TaggingManager) AddTag(targetType, targetID, tagName string) error {
	...
	return tm.db.Where(Tag{Name: tagName, Type: targetType, TargetID: targetID}).FirstOrCreate(&tag).Error
}

// RemoveTag 为目标移除标签
func (tm *TaggingManager) RemoveTag(targetType, targetID, tagName string) error {
	return tm.db.Where("type = ? AND target_id = ? AND name = ?", targetType, targetID, tagName).Delete(&Tag{}).Error
}
