...
func main() {
	...
	// 4. 初始化 AI 服务和 RAG
	aiService := app.NewAIService(db, nil)
	es := rag.NewTaskAIEmbeddingService(aiService, embedModel.ID, embedModel.ModelID)
	kb := rag.NewPostgresKnowledgeBase(db, es)

	// 5. 准备测试数据
	// 首先尝试索引真实的项目文档
	indexProjectDocs(db, kb)
	// 如果还是没数据，则注入模拟数据
...
func indexProjectDocs(db *gorm.DB, kb *rag.PostgresKnowledgeBase) {
	indexer := rag.NewIndexer(kb)
