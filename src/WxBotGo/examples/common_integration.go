...
type WxConnectionHandler struct {
	config bot.ConnectionConfig
	running bool
}

// NewWxConnectionHandler 创建微信连接处理器
func NewWxConnectionHandler(config bot.ConnectionConfig) *WxConnectionHandler {
...
// Start 启动微信连接
func (h *WxConnectionHandler) Start(config bot.ConnectionConfig) error {
...
func InitConnectionManager() (*bot.ConnectionManager, error) {
	// 创建连接管理器
	manager := bot.NewConnectionManager()
