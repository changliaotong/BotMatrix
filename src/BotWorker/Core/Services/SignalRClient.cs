...
namespace BotWorker.Core.Services
{
    public class SignalRClient
    {
        ...
        private void StartPrimaryCheckTimer()
        {
            _primaryCheckTimer = new Timer(async _ =>
            {
                ...
                try
                {
                    ...
                    InfoMessage("[主服务器] 主服务器恢复，切换回主服务器...");
                    await SwitchToPrimaryAsync(); 
                }
                catch
                {
                    InfoMessage("[主服务器] 主服务器仍不可用...");
                }
            }, null, TimeSpan.FromSeconds(15), TimeSpan.FromSeconds(30)); // 每 30 秒检测一次
        }
        ...
        private async Task SwitchToServerAsync(int index)
        {
            ...
            InfoMessage($"[SignalR] 已连接{(_currentServerIndex == 0 ? "主" : "备用")}服务器：{url}");
        ...
        public SignalRClient(long userId, List<string> serverUrls, Action<string>? log = null)
        {
            if (serverUrls == null || serverUrls.Count == 0)
                throw new ArgumentException("至少提供一个服务器地址");
        ...
        private void RegisterHandlerDynamic(string methodName, Delegate handler)
        {
            ...
            if (methodInfo == null)
                throw new Exception("[SignalR] 找不到合适的 On 方法");
        ...
        private async Task<T?> SendMessageAsync<T>(string method, params object[] args)
        {
            ...
            {
                InfoMessage($"[SignalR] 发送失败：{ex.GetBaseException()}");
            }
        ...
        private void AttachConnectionEvents()
        {
            _conn.Closed += async (error) =>
            {
                if (_isManualStop)
                {
                    InfoMessage("[SignalR] 手动断开，忽略自动重连");
                    return;
                }

                InfoMessage($"[SignalR] 连接关闭：{error?.Message}");
            ...
            _conn.Reconnected += async (_) =>
            {
                ...
                InfoMessage("[SignalR] 已重连，尝试重发缓存消息");
                await RetryAllAsync();
            };
        }
        ...
        public async Task RetryAllAsync()
        {
            ...
            {
                InfoMessage("[SignalR] 当前未连接，跳过重试");
                return;
            }
            ...
            foreach (var msg in toRetry)
            {
                try
                {
                    ...
                    InfoMessage($"[SignalR] 重试成功：{msg.MethodName}");
                }
                catch (Exception ex)
                {
                    InfoMessage($"[SignalR] 重试失败：{ex.Message}");
                }
            }
            ...
            try
            {
                await RetryHelper.RetryAsync(async () =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            InfoMessage("[SignalR] 正在停止连接");
                            await StopManuallyAsync();
                        }
                    }
                    catch (Exception ex)
                    {
                        ErrorMessage($"[SignalR] Stop 失败：{ex.Message}");
                    }
                    finally
                    {
                    ...
                    InfoMessage($"[SignalR] 尝试连接{(_currentServerIndex == 0 ? "主" : "备用")}服务器：{_serverUrls[_currentServerIndex]}");
                ...
                {
                    InfoMessage($"[SignalR] 连接失败，{delay} 秒后重试...");
                    await SwitchToBackupAsync();
                });

                InfoMessage("[SignalR] 连接成功");
            }
            finally
            {
