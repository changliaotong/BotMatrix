...
namespace BotWorker.Core.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<int> AddCoinsAsync(int coinsType, long coinsValue, long groupId, long userId, string reason)
        {
            ...
            // 3. 构建原子更新任务
            // 使用 TaskPlus 代替手动 SQL，这样可以利用我们的 ReSync 机制更新局部缓存
            var updateTask = Groups.GroupMember.TaskPlus(fieldName, coinsValue, groupId, userId);
            ...
            // 5. 开启事务执行
            // ExecTrans 会自动检测 SqlTask 中的 NeedsReSync 并在成功后重读数据库同步 Redis
