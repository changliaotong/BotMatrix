...
namespace BotWorker.Modules.Games
{
    public class RedBlueService : IRedBlueService
    {
        ...
        public async Task<string> GetRedBlueResAsync(BotMessage botMsg, bool isDetail = true)
        {
            botMsg.IsCancelProxy = true;

            if (!botMsg.Group.IsCreditSystem)
                return BotMessage.CreditSystemClosed;

            if (await botMsg.IsTooFastAsync())
                return BotMessage.RetryMsgTooFast;

            using var wrapper = await botMsg.BeginTransactionAsync();
            try
            {
                // 1. 在事务内获取积分并锁定用户
                long creditValue = await _userCreditService.GetCreditForUpdateAsync(botMsg.SelfId, botMsg.GroupId, botMsg.UserId, wrapper.Transaction);

                // 2. 在事务内加载并锁定牌堆
                List<Card> deck = await _shuffledDeckRepo.ReadShuffledDeckAsync(botMsg.GroupId, wrapper.Transaction, true);
                ...
                if (deck.Count < 6)
                {
                    ...
                    // 保存新生成的牌堆
                    await _shuffledDeckRepo.ClearShuffledDeckAsync(botMsg.GroupId, wrapper.Transaction);
                    foreach (var (card, i) in deck.Select((c, i) => (c, i)))
                    {
                        ...
                        await wrapper.Transaction.Connection.InsertAsync(item, wrapper.Transaction);
                    }
                }

                if (!botMsg.CmdPara.IsNum())
                {
                    if (botMsg.CmdPara.ToUpper().In("梭哈", "SH"))
                        botMsg.CmdPara = creditValue.ToString();
                    else
                    {
                ...
                long blockCredit = botMsg.CmdPara.AsLong();
                if (blockCredit < botMsg.Group.BlockMin)
                {
                    await wrapper.RollbackAsync();
                    return $"至少押{botMsg.Group.BlockMin}分";
                }
                ...
                // 发牌
                List<Card> playerHand = [deck[0], deck[2]]; // 第1张和第3张牌给蓝方 (注意：RedBlueMessage 里的注释说第1和3给红方，但逻辑上 player 是蓝，banker 是红)
                List<Card> bankerHand = [deck[1], deck[3]]; // 第2张和第4张牌给红方
                ...
                await _shuffledDeckRepo.ClearShuffledDeckAsync(botMsg.GroupId, usedCardIds, wrapper.Transaction);
                ...
                bool isWin = result.Contains(botMsg.CmdName);
                long creditGet = 0;
                long creditAdd;
                if (isWin)
                {
                    int odds = payout;
                    if (botMsg.CmdName == "红" && RedBlue.CalculateTotal(bankerHand) == 6)
                ...
                if (creditAdd != 0)
                {
                    var addRes = await _userCreditService.AddCreditAsync(botMsg.SelfId, botMsg.GroupId, botMsg.GroupName, botMsg.UserId, botMsg.Name, creditAdd, "红和蓝得分", wrapper.Transaction);
                    if (addRes.Result == -1)
                    {
                        await wrapper.RollbackAsync();
                        return BotMessage.RetryMsg;
                    }
                ...
                // 同步缓存
                await _userCreditService.SyncCreditCacheAsync(botMsg.SelfId, botMsg.GroupId, botMsg.UserId, creditValue);
            ...
            {
                await wrapper.RollbackAsync();
                _logger.LogError(ex, "GetRedBlueRes Error");
                return BotMessage.RetryMsg;
            }
        }
    }
