...
namespace BotWorker.Modules.Buses
{
    ...
    public class Bus : MetaData<Bus>
    {
        ...
        {
            if (busName.Contains("换乘"))
                return "温馨提示：斜体显示站点需要换乘才能到达";
            else
                return "温馨提示：此站点为单边停靠站点，反方向不停靠此站点";
        }

        public static string GetEmInfo(string busName)
        {
            if (busName.Contains("换乘"))
                return "温馨提示：<em>斜体显示站点为换乘后途径站点</em>";
            else
                return "温馨提示：<em>斜体显示站点为单方向停靠站点，反方向不停靠</em>";
        }
        ...
        //由 keyword 得到符合条件的第一条线路的 bus_id
        ...
        //获得线路 bus_id 线路从 palce_id 到 palce_id2 经过站点数
        ...
        {
            return $"kresult+=\"mapa|mapb|\";var mapa= new Array(\"去程\",{GetBusMapA(bus_id)});var mapb= new Array(\"回程\",{GetBusMapB(bus_id)});";
        }
