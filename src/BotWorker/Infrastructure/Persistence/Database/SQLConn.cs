...
namespace BotWorker.Infrastructure.Persistence.Database
{
    ...
    {        
        ...
        {
            return QueryList<T>(sql, null, parameters);
        }

        public static List<T>? QueryList<T>(string sql, IDbTransaction? trans = null, params IDataParameter[] parameters)
        {
            return QueryList<T>(sql, true, trans, parameters);
        }

        public static List<T>? QueryList<T>(string sql, bool isDebug = true, params IDataParameter[] parameters)
        {
            return QueryList<T>(sql, isDebug, null, parameters);
        }

        public static List<T>? QueryList<T>(string sql, bool isDebug, IDbTransaction? trans, params IDataParameter[] parameters)
        {
            var json = QueryAsJson(sql, isDebug, trans, parameters);
            return JsonConvert.DeserializeObject<List<T>>(json);
        }

        public static string QueryAsJson(string sql, bool isDebug, params IDataParameter[] parameters)
        {
            return QueryAsJson(sql, isDebug, null, parameters);
        }

        public static string QueryAsJson(string sql, bool isDebug, IDbTransaction? trans, params IDataParameter[] parameters)
        {
            // 如果没有传递参数，则使用一个空数组
            parameters ??= [];

            DataSet ds = QueryDataset(sql, trans, parameters);
        ...
        public static async Task<long> GetAutoIdAsync(string tableName)
        {
            string sql = GlobalConfig.DbType == DatabaseType.SqlServer ? "SELECT SCOPE_IDENTITY()" : "SELECT lastval()";
