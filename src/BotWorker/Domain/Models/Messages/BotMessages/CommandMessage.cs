...
public partial class BotMessage : MetaData<BotMessage>
{
        public async Task GetCmdResAsync()
        {
            ...
            { 
                ...
                Answer = Chengyu.GetFanChaRes(this).ReplaceInvalid();
            else if (CmdName == "身份证")
            ...
            else if (!CmdName.IsNull())
            {
                ...
                if (CmdName.In("开机", "关机") && CmdPara.IsNull())                
                    Answer = GroupInfo.SetPowerOnOff(SelfId, GroupId, UserId, CmdName);               
                else if (CmdName.In("开启", "关闭"))
                {
                    ...
                    Answer = GroupWarn.GetWarnInfoAsync(GroupId, CmdPara);
                else if (CmdName == "清警告")
                    Answer = GroupWarn.GetClearRes(GroupId, CmdPara);
