...
namespace BotWorker.Domain.Models.Messages.BotMessages
{
    public partial class BotMessage : MetaData<BotMessage>
    {
        ...
        public async Task<string> GetWarnRes()
        {
            ...
            if (GroupWarn.AppendWarn(SelfId, qqWarn, GroupId, "", UserId) == -1)
                return RetryMsg;

            long countWarn = GroupWarn.WarnCount(qqWarn, GroupId);
            IsCancelProxy = true;
            if (countWarn >= Group.BlackCount)
            {
                int i = BlackList.AddBlackList(SelfId, GroupId, GroupName, UserId, Name, qqWarn, "警告超限拉黑");
                if (i == -1)
                    return RetryMsg;

                i = GroupWarn.ClearWarn(GroupId, qqWarn);
                if (i == -1)
                    ErrorMessage($"清空警告{RetryMsg}");
        ...
        public async Task GetKeywordWarnAsync()
        {
            ...
            if (!regexKey.IsNullOrEmpty())
            {
                ...
                if (isMatch)
                {
                    BlackList.AddBlackList(SelfId, GroupId, GroupName, SelfId, SelfName, UserId, "敏感词拉黑");
                    GroupWarn.ClearWarn(GroupId, UserId);
            ...
            if (!regexKey.IsNullOrEmpty())
            {
                ...
                if (isMatch)
                {
                    GroupWarn.AppendWarn(SelfId, UserId, GroupId, Message, SelfId);
                    await RecallAsync(SelfId, RealGroupId, MsgId);
                    await KickOutAsync(SelfId, RealGroupId, UserId);
