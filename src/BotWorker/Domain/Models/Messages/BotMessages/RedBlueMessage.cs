...
namespace BotWorker.Domain.Models.Messages.BotMessages
{
    ...
    public partial class BotMessage : MetaData<BotMessage>
    {
        public async Task<string> GetRedBlueResAsync(bool isDetail = true)
        {
            ...
            if (creditAdd != 0)
            {
                var addRes = await UserInfo.AddCreditAsync(SelfId, GroupId, GroupName, UserId, Name, creditAdd, "红和蓝得分");
                if (addRes.Result == -1)
                    return RetryMsg;
        ...
        public string GetRedBlueRes(bool isDetail = true)
        {
            ...
            if (creditAdd != 0)
            {
                var addRes = UserInfo.AddCredit(SelfId, GroupId, GroupName, UserId, Name, creditAdd, "红和蓝得分");
                if (addRes.Item1 == -1)
                    return RetryMsg;
                creditValue = addRes.Item2;
            }
