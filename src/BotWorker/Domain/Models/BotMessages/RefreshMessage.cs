namespace BotWorker.Domain.Models.BotMessages
{
    public partial class BotMessage : MetaData<BotMessage>
    {
        ...
        public async Task GetRefreshRes()
        {
            ...
            Answer = $"请勿刷屏{times}";
            Answer += isWarn ? await AddWarn(UserId, Name, SelfId) : "";
            GroupEvent.Append(this, $"警告", $"刷屏警告");

            if (isMinus)
            {
                (int i, long creditValue) = await MinusCreditAsync((long)(Math.Pow(2, times - 1) * 10), "刷屏扣分");
                Answer = i == -1
                    ? "刷屏扣分失败"
        ...
        public async Task HandleRefreshAsync()
        {
            ...
            if (times >= 20)
            {
                int i = await BlackList.AddBlackListAsync(SelfId, BotInfo.GroupIdDef, $"{GroupName}({GroupId})", SelfId, "机器人", UserId, $"群{GroupId}内刷屏拉黑");
                if (i != -1)
                {
            ...
            else
            {
                ...
                (int i, long creditValue) = await MinusCreditAsync(minus, "刷屏扣分");
                if (i != -1)
                {
