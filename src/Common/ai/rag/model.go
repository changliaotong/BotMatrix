...
type KnowledgeDocAccess struct {
	...
	OwnerType string    `gorm:"size:20;index" json:"owner_type"` // system, user, group, bot
...
type KnowledgeChunk struct {
	ID        uint          `gorm:"primaryKey" json:"id"`
	DocID     uint          `gorm:"index" json:"doc_id"`
	ParentID  uint          `gorm:"index" json:"parent_id"` // 父切片 ID (用于 Parent-Child 检索)
	...
	Type      string        `gorm:"size:20;index;default:'chunk'" json:"type"` // parent, chunk, small
	CreatedAt time.Time     `json:"created_at"`
}
...
type KnowledgeRelation struct {
	...
	Predicate   string           `gorm:"size:100;index" json:"predicate"` // 关系谓词
	Description string           `gorm:"type:text" json:"description"`
	Metadata    string           `gorm:"type:jsonb" json:"metadata"`
	CreatedAt   time.Time        `json:"created_at"`
}
