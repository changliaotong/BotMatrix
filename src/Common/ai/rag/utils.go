...
func DescribeImage(ctx context.Context, svc AIService, modelID uint, imageContent []byte) (string, error) {
	if svc == nil {
	...
	if parts, ok := choice.Message.Content.([]types.ContentPart); ok {
...
func RecursiveCharacterTextSplitter(text string, chunkSize, chunkOverlap int, separators []string) []string {
	...
	for _, split := range splits {
		if currentLen+len(split) > chunkSize && currentLen > 0 {
			doc := strings.Join(currentDoc, separator)
			finalChunks = append(finalChunks, doc)

			// 处理 overlap
			// 简单处理：保留最后一部分
			overlapStart := len(currentDoc) / 2
			if overlapStart < len(currentDoc) {
				currentDoc = currentDoc[overlapStart:]
				currentLen = 0
				for _, s := range currentDoc {
					currentLen += len(s)
				}
			} else {
				currentDoc = nil
		...
		if len(split) > chunkSize {
			// 如果单个分割块已经超过 chunkSize，递归使用下一个分隔符
			if len(nextSeparators) > 0 {
				subChunks := RecursiveCharacterTextSplitter(split, chunkSize, chunkOverlap, nextSeparators)
	...
	if len(currentDoc) > 0 {
		finalChunks = append(finalChunks, strings.Join(currentDoc, separator))
	}

	return finalChunks
}

// SemanticChunker 使用 AI 对文本进行语义聚类或切分
func SemanticChunker(ctx context.Context, svc types.AIService, modelID uint, content string) []Chunk {
	if svc == nil {
		return []Chunk{{Content: content}}
	}

	prompt := `你是一个文档处理专家。请将以下文本按语义逻辑切分成多个片段。
每个片段应该是一个完整的语义单元（如一个步骤、一个概念解释或一个功能描述）。
请直接输出切分后的片段，每个片段之间用 "---CHUNK_BOUNDARY---" 分隔。
不要包含任何额外的解释或 Markdown 格式。

文本内容：
` + content

	resp, err := svc.Chat(ctx, modelID, []types.Message{
	...
	parts := strings.Split(resStr, "---CHUNK_BOUNDARY---")
	var chunks []Chunk
	for _, p := range parts {
...
// HierarchicalParser 装饰器，为解析器增加多级切分功能
type HierarchicalParser struct {
	BaseParser ContentParser
	ChildSize  int
}

func (p *HierarchicalParser) Parse(ctx context.Context, content []byte) []Chunk {
	parentChunks := p.BaseParser.Parse(ctx, content)
	for i := range parentChunks {
		// 对每个父片段进行二次切分
		childTexts := RecursiveCharacterTextSplitter(parentChunks[i].Content, p.ChildSize, p.ChildSize/10, nil)
		if len(childTexts) > 1 {
			for _, ct := range childTexts {
				parentChunks[i].Children = append(parentChunks[i].Children, Chunk{
					Content: ct,
					Title:   parentChunks[i].Title,
				})
			}
		}
	}
...
// MarkdownParser Markdown 解析器实现 (增强版)
...
func (p *MarkdownParser) Parse(ctx context.Context, content []byte) []Chunk {
	// 先按标题切分
	chunks := SimpleMarkdownChunker(string(content), p.MinSize)
	// 如果某个片段还是太大，递归切分
	var finalChunks []Chunk
	for _, c := range chunks {
		if len(c.Content) > 2000 {
			subTexts := RecursiveCharacterTextSplitter(c.Content, 1000, 100, nil)
			for _, st := range subTexts {
				finalChunks = append(finalChunks, Chunk{Content: st, Title: c.Title})
			}
		} else {
...
func (p *DocxParser) Parse(ctx context.Context, content []byte) []Chunk {
	...
	// Docx 通常按段落切分，或者直接使用 SimpleMarkdownChunker 的 logic (如果包含标题)
