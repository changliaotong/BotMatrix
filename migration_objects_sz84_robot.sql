-- Object: sp_TransferCoins

 
CREATE OR REPLACE PROCEDURE "dbo"."sp_TransferCoins"(@robot_qq bigint, @group_id bigint, @group_name varchar(50), @from_qq bigint, @to_qq bigint, @amount money, @transfer_info varchar(200))
AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE "dbo"."sp_GetLists"
@tblName varchar(200), 
@idName varchar(200), 
...
set @sql = 'select top ' + convert(varchar(12),@pagesize) + 
...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT @client_credit = COALESCE(credit, 0), @save_credit = COALESCE(savecredit, 0), @tokens = COALESCE(tokens, 0)
		FROM "User"
		WHERE Id = @client_qq;
...
BEGIN
    ...
    BEGIN TRY
		...
		select @group_owner = GroupOwner from "group" where Id = COALESCE((select top 1 GroupId from vip where groupid = @target_group), @group_id)
...
-- Object: sp_UpdateWeixinQQ

CREATE OR REPLACE PROCEDURE "dbo"."sp_UpdateWeixinQQ"(@client_qq bigint, @client_qq2 bigint)
AS $$
BEGIN
...
-- Object: sp_BuyVIP
CREATE OR REPLACE PROCEDURE "dbo"."sp_BuyVIP"(@group_id bigint,@client_qq bigint,@c_month int,@pay_method varchar(50),@pay_money money, @vip_trade varchar(50), @vip_info varchar(200), @insert_by int)
AS $$
BEGIN
	...
	select @vip_id=id,@end_date=enddate,@is_year_vip=DATEDIFF(MONTH,NOW(),enddate) from vip where GroupId = @group_id
	select @is_year_vip = COALESCE(@is_year_vip,0) + @c_month
	if (@is_year_vip >= 11) 
	...
	begin
		...
		values(@group_id,@group_name,@pay_money,NOW(),dateadd(month,@c_month,NOW()),@vip_info,@client_qq,@pay_money/(@c_month*1.0),@is_year_vip,@insert_by)	
	end else
	begin
		--已经开过vip的群更新有效期
		update vip set enddate = dateadd(month, @c_month, enddate),vipinfo=@vip_info, UserId = @client_qq,
		incomeday = (incomeday * DATEDIFF(MONTH,NOW(),enddate) + @pay_money) /((DATEDIFF(MONTH,NOW(),enddate) + @c_month + 0.0000001)*1.0),
...
-- Object: StringAggregateMultipleColumns
...
BEGIN
    ...
    IF LENGTH(@result) > 0
        SET @result = STUFF(@result, LENGTH(@result) - LENGTH(@delimiter) + 1, LENGTH(@delimiter), '')
...
-- Object: getMinRobotEnt
 
CREATE OR REPLACE FUNCTION "dbo"."getMinRobotEnt"() 
RETURNS bigint
AS $$
BEGIN
	declare @MinRobot bigint
...
AS
RETURN 
(select a.groupid, COALESCE(a.groupname, '') as group_name,trialstartdate, trialenddate, c_audit, c_all,
startdate, enddate, COALESCE(is_vip, 0) as is_vip, isvalid, COALESCE(c_msg, '') as c_msg from 
...
BEGIN
	...
	select @c_all = COALESCE(COUNT(id),0) from chengyu where groupid = @group_id and id >= @max_game_id
	select @c_client = COALESCE(COUNT(id),0) from chengyu where groupid = @group_id and userid = @client_qq and id >= @max_game_id
	if (@group_id = 0)
		return (@c_all - 1)/2	
...
BEGIN
    ...
    SET @sql = 'SELECT TOP ' + CONVERT(VARCHAR(12), @PageSize) + ' ' + @sSelect + 
        ' FROM ' + @tblName + ' WHERE ' + @idName + 
...
-- Object: getMinRobotEntFree

CREATE OR REPLACE FUNCTION "dbo"."getMinRobotEntFree"() 
RETURNS bigint
AS $$
BEGIN
	declare @MinRobot bigint
...
-- Object: sp_UpdatePublicQQ

CREATE OR REPLACE PROCEDURE "dbo"."sp_UpdatePublicQQ"(@client_qq bigint, @client_qq2 bigint)
AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE "dbo"."sp_AddCoins"(@robot_qq bigint, @group_id bigint, @group_name varchar(50), @client_qq bigint, @client_name varchar(50), @coins_add money, @coins_info varchar(200))
AS $$
BEGIN
	if @group_name = ''
	begin
...
-- Object: sp_BuyCredit
CREATE OR REPLACE PROCEDURE "dbo"."sp_BuyCredit"(@robot_qq bigint, @client_qq bigint,@credit_add int,@pay_method varchar(50),@pay_money money, @credit_trade varchar(50), @credit_info varchar(200), @insert_by int)
AS $$
BEGIN
	declare @client_name varchar(50)
	declare @ref_qq bigint
	
	select @client_name = name, @ref_qq = COALESCE(RefUserId, 0) from "user" where Id = @client_qq
...
-- Object: sp_AddCredit
CREATE OR REPLACE PROCEDURE "dbo"."sp_AddCredit"(@robot_qq bigint, @client_qq bigint,@credit_add int, @credit_info varchar(200))
AS $$
BEGIN
	declare @client_name varchar(50)
	select @client_name = name from "User" where Id = @client_qq
	...
	update "User" set Credit=COALESCE(credit,0) + @credit_add where Id = @client_qq	
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE PROCEDURE "dbo"."sp_ChangeVip"(@group_id bigint, @new_group_id bigint, @new_client_qq bigint, @change_by int)
AS $$
BEGIN
...
-- Object: sp_BuyCoins
CREATE OR REPLACE PROCEDURE "dbo"."sp_BuyCoins"(@robot_qq bigint, @client_qq bigint, @coins_add int, @pay_method varchar(50),@pay_money money, @coins_trade varchar(50), @coins_info varchar(200), @insert_by int)
AS $$
BEGIN
	declare @client_name varchar(50)
	select @client_name = name from "User" where Id = @client_qq		
